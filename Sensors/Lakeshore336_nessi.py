...
def open_log():
    ...
    fname = "/var/tempLogs/LakeShoreLog_" + strftime("%Y%m%d %H:%M:%S", gmtime()) + ".txt"
    with open(fname, "a") as text_file:
        text_file.write('Date, Time, Epoch, Shield, Tank, Mask, Array\n')
...
def make_plot(fname):
    ...
    legend((l1), ('Shield', 'Tank', 'Mask', 'H2RG'), 'upper center', shadow=True)
